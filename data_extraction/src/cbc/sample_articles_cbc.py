...
def cbc_sampling_wrapper(economic_indicator,project_path, sample_size,column_names_list = ['title', 'description','publishedAt'],source_string = "cbc", remove_Nans = True):
    ...
    try:
        ...
        json_file_path =  "data_extraction/data/unannotated_data/" + source_string.lower() + "/" +  economic_indicator + "_" + source_string.lower() + "_article.json"
...
def unit_tests():
    ...
    project_path = "/Users/nagasiri/Desktop/NagaSiri/MDS-CL/Capstone/better_dwelling_capstone/project/"
    rt = cbc_sampling_wrapper("mortgagerates",project_path, -1, ['title', 'description','publishedAt'],"cbc")
    assert rt == False, "function should return false if sample size is -1 or less"
